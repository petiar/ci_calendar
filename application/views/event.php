...
<table class="table">
  ...
  <tr>
    ...
    <td><?php if (($this->rsvp_model->can_edit($row->id)) || spm_is_admin()): ?>
        <a href="<?php print site_url('events/subscribe/' . $event['id'] . '/' . $row->id); ?>"><i class="fas fa-edit"></i></a>
      <?php endif; ?></td>
    <td>
      <?php if (($this->rsvp_model->can_edit($row->id)) || spm_is_admin()): ?>
          <a href="<?php print site_url('events/confirm/' . $event['id'] . '/' . $row->id); ?>"><i class="fas fa-trash-alt"></i></a>
      <?php endif; ?>
    </td>
  </tr>
  <?php endforeach; ?>
